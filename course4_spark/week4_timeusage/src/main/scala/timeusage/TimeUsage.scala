...
//    // ---- by dataset
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val fields =
      for { i <- 0 until columnNames.length
        ...
        if (i==0)
          StructField(columnNames(i), StringType, false)
        else
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val cols = columnNames.groupBy(name => {
      if(primaryPrefix.exists(p => name.startsWith(p)))
        "primary"
      else if(workPrefix.exists(p => name.startsWith(p)))
        "working"
      else if(leisurePrefix.exists(p => name.startsWith(p)))
        "leisure"
      else
        "other"
    })

    (cols("primary").map(x => col(x)),
     cols("working").map(x => col(x)),
     cols("leisure").map(x => col(x))
    )
  }
  ...
  def timeUsageSummary(
    ...
    val primaryNeedsProjection: Column = (primaryNeedsColumns.reduce((a, b) => a + b)/60).as("primaryNeeds")
