...
object Huffman {
  ...
  case class Fork(left: CodeTree,
    ...
    override def toString: String =
      "(" + left.toString + "," + weight + "," + right.toString + ")"
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    trees match {
      ...
      case t1 :: t2 :: tail =>  {
        ...
        val list = (fork :: tail).sortBy(x => weight(x))
        combine(list)
      }
    }
  }
  ...
  /**
    ...
    */
  def until(fn1: List[CodeTree] => Boolean,
            fn2: List[CodeTree] => List[CodeTree])
           (trees: List[CodeTree]): List[CodeTree] = {
    def iter(trees: List[CodeTree]): List[CodeTree] =
      if (fn1(trees))
        trees
      else
        iter(fn2(trees))
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def walk(root: CodeTree, tree: CodeTree, index: Int, acc: List[Char]): List[Char] = {
      tree match {
        case Leaf(c, w) =>
          if(index == bits.length)
            acc :+ c
          else
            walk(root, root, index, acc:+ c)
        case Fork(left, right, fchars, w) =>
          if (bits(index) == 0)
            walk(root, left, index + 1, acc)
          else
            walk(root, right, index + 1, acc)
      }
    }

    walk(tree, tree, 0, List())
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def dfs(acc:CodeTable, tree: CodeTree, bits: List[Bit]): CodeTable = {
      tree match {
        case Leaf(c, w) => acc ::: List((c, bits))
        case Fork(left, right, chars, w) => dfs(acc, left, bits :+ 0) ::: dfs(acc, right, bits :+ 1)
      }
    }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    val init = List[Bit]()
    text.foldLeft( init )( (acc, c) => acc ::: codeBits(table)(c) )
  }
}
