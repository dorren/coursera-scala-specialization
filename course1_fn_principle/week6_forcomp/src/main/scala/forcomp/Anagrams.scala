...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase.groupBy(_.toChar).mapValues(_.size).toList.sortBy(_._1)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    x.foldLeft(Nil: Occurrences)( (acc, pair) => {
      val idx = y.indexWhere(_._1 == pair._1)

      if(idx != -1) {
        val item = y(idx)
        if(pair._2 - item._2 > 0)
          acc :+ (pair._1, pair._2 - item._2)
        else
          acc    // count 0, skip
      }else {
        acc :+ pair
    ...
    def iter(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) {
        ...
        for {
          ...
          sentence1 <- iter(subtract(occurrences, wordOccurrences(word)))
          if !comb.isEmpty
        } yield {
          word :: sentence1
        }
